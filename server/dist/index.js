...
const stringSessionPath = path_1.default.join(__dirname, '../../session.txt');
let stringSession = '';
if (fs_1.default.existsSync(stringSessionPath)) {
    stringSession = fs_1.default.readFileSync(stringSessionPath, 'utf8').trim();
}
const client = new telegram_1.TelegramClient(new sessions_1.StringSession(stringSession), Number(API_ID), API_HASH || '', { connectionRetries: 3 });
...
app.post('/uploader/metube/cookies', upload.single('cookies'), async (req, res) => {
    try {
        ...
        try {
            ...
            exec('systemctl restart metube', (err) => {
                if (err)
                    console.error('metube restart error (async)', err);
            });
        }
        catch (e) {
            console.error('failed to spawn restart', e);
        }
        res.send('ok');
    }
    catch (e) {
...
app.post('/metube/cookies', upload.single('cookies'), async (req, res) => {
    try {
        ...
        try {
            ...
            exec('systemctl restart metube', (err) => {
                if (err)
                    console.error('metube restart error (async alias)', err);
            });
        }
        catch (restartErr) {
...
const JOBS_PATH = path_1.default.join(__dirname, '../../jobs.json');
function saveJobsToDisk() {
    try {
        const arr = Array.from(jobs.values());
        const tmp = JOBS_PATH + '.tmp';
        fs_1.default.writeFileSync(tmp, JSON.stringify(arr, null, 2), { mode: 0o600 });
        fs_1.default.renameSync(tmp, JOBS_PATH);
    }
    catch (e) {
...
function loadJobsFromDisk() {
    try {
        if (!fs_1.default.existsSync(JOBS_PATH))
            return;
        const raw = fs_1.default.readFileSync(JOBS_PATH, 'utf8');
...
app.delete('/jobs/:id', (req, res) => {
    ...
    try {
        const DL_DIR = path_1.default.resolve('/var/www/dl');
        if (job.tmpPath && fs_1.default.existsSync(job.tmpPath)) {
            const resolved = path_1.default.resolve(job.tmpPath);
            const inside = resolved === DL_DIR || resolved.startsWith(DL_DIR + path_1.default.sep);
...
// free space
function freeSpaceHandler(req, res) {
    try {
...
function dlListHandler(req, res) {
    try {
        ...
        res.setHeader('Surrogate-Control', 'no-store');
        const DL_DIR = path_1.default.resolve('/var/www/dl');
        if (!fs_1.default.existsSync(DL_DIR))
            return res.json({ items: [], total: 0 });
        const q = (req.query.q || '').toString().toLowerCase();
        const page = Math.max(1, Number(req.query.page || 1));
        const perPage = Math.max(5, Math.min(200, Number(req.query.perPage || 50)));
        const all = fs_1.default.readdirSync(DL_DIR).map(f => {
            try {
                const st = fs_1.default.statSync(path_1.default.join(DL_DIR, f));
                return { name: f, size: st.size, mtime: st.mtimeMs, isFile: st.isFile() };
            }
            catch (e) {
                return null;
            }
        }).filter(Boolean).filter(x => x.isFile);
        let filtered = all;
        if (q)
            filtered = all.filter(it => it.name.toLowerCase().includes(q));
...
function buildDlTree(rootAbs, rel, limiter) {
    const abs = path_1.default.join(rootAbs, rel);
    let entries = [];
    try {
        entries = fs_1.default.readdirSync(abs);
    }
    catch {
        return [];
    }
    ...
    for (const name of entries) {
        if (limiter.count >= limiter.max)
            break;
        const full = path_1.default.join(abs, name);
        let st;
        try {
        ...
        const node = {
            name,
            path: path_1.default.posix.join(rel.split(path_1.default.sep).filter(Boolean).join('/'), name).replace(/\\/g, '/'),
            mtime: st.mtimeMs,
        ...
        limiter.count++;
        if (node.isDir) {
            node.children = buildDlTree(rootAbs, path_1.default.join(rel, name), limiter);
        }
    }
...
function dlTreeHandler(req, res) {
    try {
        res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate');
        res.setHeader('Pragma', 'no-cache');
        res.setHeader('Expires', '0');
...
function dlDeleteHandler(req, res) {
    try {
        ...
        const DL_DIR = path_1.default.resolve('/var/www/dl');
        const resolved = path_1.default.resolve(DL_DIR, name);
        if (!(resolved === DL_DIR || resolved.startsWith(DL_DIR + path_1.default.sep))) {
            return res.status(400).send('invalid path');
        }
        if (!fs_1.default.existsSync(resolved))
            return res.status(404).send('not found');
        const st = fs_1.default.statSync(resolved);
        if (!st.isFile())
...
app.delete('/uploader/dl/any', (req, res) => {
    try {
        const { username, password, path: relPath } = (req.body || {});
        if (username !== UI_USERNAME || password !== UI_PASSWORD) {
            return res.status(401).send('Unauthorized');
        }
        ...
        const DL_DIR = path_1.default.resolve('/var/www/dl');
        const target = path_1.default.resolve(DL_DIR, relPath);
        if (!(target === DL_DIR || target.startsWith(DL_DIR + path_1.default.sep))) {
            return res.status(400).send('invalid path');
        }
        if (!fs_1.default.existsSync(target))
            return res.status(404).send('not found');
        const st = fs_1.default.statSync(target);
        if (st.isDirectory()) {
...
app.post('/uploader/dl/rename', (req, res) => {
    try {
        const { username, password, oldPath, newPath } = (req.body || {});
        if (username !== UI_USERNAME || password !== UI_PASSWORD) {
            return res.status(401).send('Unauthorized');
        }
        ...
        const DL_DIR = path_1.default.resolve('/var/www/dl');
        const oldTarget = path_1.default.resolve(DL_DIR, oldPath);
        const newTarget = path_1.default.resolve(DL_DIR, newPath);
        if (!(oldTarget === DL_DIR || oldTarget.startsWith(DL_DIR + path_1.default.sep))) {
            return res.status(400).send('invalid oldPath');
        }
        if (!(newTarget === DL_DIR || newTarget.startsWith(DL_DIR + path_1.default.sep))) {
            return res.status(400).send('invalid newPath');
        }
        if (!fs_1.default.existsSync(oldTarget)) {
            return res.status(404).send('oldPath not found');
        }
        if (fs_1.default.existsSync(newTarget)) {
            return res.status(409).send('newPath already exists');
        }
        fs_1.default.renameSync(oldTarget, newTarget);
        res.send('renamed');
    }
    catch (e) {
...
function sendSse(jobId, event, payload) {
...
function throttledProgress(jobId, kind, data) {
    const now = Date.now();
    const rec = lastProgressSent[jobId] || { t: 0 };
    const minInterval = 750; // ms between emits
    let changed = false;
    if (kind === 'download') {
        if (data.percent != null && data.percent !== rec.percent)
            changed = true;
        if (rec.t === 0)
            changed = true;
    }
    ...
    if (!changed && now - rec.t < minInterval)
        return; // skip
...
app.post('/auth/start', async (req, res) => {
    ...
    try {
        await client.connect();
        const result = await client.invoke(new telegram_1.Api.auth.SendCode({ phoneNumber: phone, apiId: Number(API_ID), apiHash: API_HASH, settings: new telegram_1.Api.CodeSettings({}) }));
        global._phone = phone;
        global._phone_code_hash = result.phone_code_hash || result.phoneCodeHash || result.phoneCodeHash;
        res.send('code_sent');
    }
    catch (err) {
...
app.post('/auth/verify', async (req, res) => {
    const { code } = req.body;
    const phone = global._phone;
    const phone_code_hash = global._phone_code_hash;
    if (!phone || !phone_code_hash)
        return res.status(400).send('start not called');
    try {
        const result = await client.invoke(new telegram_1.Api.auth.SignIn({ phoneNumber: phone, phoneCodeHash: phone_code_hash, phoneCode: code }));
        const saved = client.session.save();
        if (saved) {
            stringSession = saved;
            fs_1.default.writeFileSync(stringSessionPath, stringSession, { mode: 0o600 });
        }
        clientReady = true;
        res.send('ok');
    }
        ...
        if (err.code === 401 && err.errorMessage === 'SESSION_PASSWORD_NEEDED') {
            ...
            try {
                const pw = await client.invoke(new telegram_1.Api.account.GetPassword());
                ...
                const sign = await client.invoke(new telegram_1.Api.auth.CheckPassword({ password: inputCheck }));
                const saved2 = client.session.save();
                if (saved2) {
                    stringSession = saved2;
                    fs_1.default.writeFileSync(stringSessionPath, stringSession, { mode: 0o600 });
                }
                clientReady = true;
                return res.send('ok');
...
app.post('/upload', async (req, res) => {
    ...
    if (!fileUrl)
        return res.status(400).send('No fileUrl provided');
    // create a job id and respond immediately. progress will be sent over SSE at /events/:jobId
    const saveToDl = !!req.body.saveToDl;
    const jobId = `job_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    const job = { id: jobId, fileUrl, status: 'queued', createdAt: Date.now(), tmpPath: undefined };
    // record job type so UI can render it immediately
    job.type = saveToDl ? 'download' : 'upload';
    jobs.set(jobId, job);
    saveJobsToDisk();
    res.json({ jobId, type: job.type });
    // run the download/upload in background
    (async () => {
        const tmpDir = saveToDl ? '/var/www/dl' : path_1.default.join(__dirname, '../../tmp');
        ...
        if (saveToDl) {
            ...
            let baseName = path_1.default.basename(urlPath) || `download_${Date.now()}`;
            const ext = path_1.default.extname(baseName);
            const nameOnly = path_1.default.basename(baseName, ext);
            let candidate = baseName;
            let i = 1;
            while (fs_1.default.existsSync(path_1.default.join(tmpDir, candidate))) {
            ...
            fileName = `upload_${Date.now()}` + path_1.default.extname(new URL(fileUrl).pathname);
        }
        const tmpPath = path_1.default.join(tmpDir, fileName);
        try {
            const startTs = Date.now();
            const response = await axios_1.default.get(fileUrl, { responseType: 'stream', maxRedirects: 5, timeout: 0 });
            ...
            const size = fs_1.default.statSync(tmpPath).size;
            ...
            const ext = path_1.default.extname(tmpPath).toLowerCase();
            ...
            if (saveToDl) {
                ...
                const publicUrl = publicBase.replace(/\/$/, '') + '/dl/' + encodeURIComponent(path_1.default.basename(tmpPath));
            ...
            const startProgressEmitter = (method) => {
            ...
            if (clientReady) {
                ...
                try {
                    ...
                    if (stream) {
                        ...
                        if (duration && w && h) {
                            attributes = [new telegram_1.Api.DocumentAttributeVideo({ duration, w, h, supportsStreaming: true })];
                        }
                    }
                }
                catch (probeErr) {
                ...
                job.status = 'error';
                job.message = 'User client not connected and bot fallback disabled';
                jobs.set(jobId, job);
            ...
            try {
                if (!saveToDl) {
                    try {
                        const DL_DIR = path_1.default.resolve('/var/www/dl');
                        const resolved = path_1.default.resolve(tmpPath);
                        const inside = resolved === DL_DIR || resolved.startsWith(DL_DIR + path_1.default.sep);
                        console.log(`CLEANUP job ${jobId}: tmpPath=${resolved}, insideDL=${inside}`);
                        if (!inside && fs_1.default.existsSync(resolved)) {
